...
WATCH_SKILL_DIR="$(CDPATH= cd -- "$SCRIPT_DIR/../.." && pwd)"
...
# Fully configured → silent (Codex can surface status on demand via --check).
...
if [[ -z "$HAS_FFMPEG" || -z "$HAS_YTDLP" ]]; then
  echo "/watch: needs ffmpeg + yt-dlp. Run \`python3 $WATCH_SKILL_DIR/scripts/setup.py\` once to install and scaffold config."
