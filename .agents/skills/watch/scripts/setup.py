...
Modes:
  ...
  setup.py --json       Machine-readable status for Codex to parse.
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[watch] WARNING: {path} is readable by other users. " f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError:
        pass
...
def cmd_check() -> int:
    ...
    sys.stderr.write(
        f"[watch] setup incomplete ({'; '.join(parts)}). " f"Run: python3 {installer}\n"
    )
    sys.stderr.flush()
...
def cmd_install() -> int:
    ...
    print("    OPENAI_API_KEY=...  " "(fallback; get one at https://platform.openai.com/api-keys)")
    print("")
