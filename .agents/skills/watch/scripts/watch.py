...
Prints a markdown report to stdout listing frame paths + transcript. Codex
then inspects each frame path with its local image-viewing capability.
"""
...
def main() -> int:
    ...
    if meta.get("width") and meta.get("height"):
        print(
            f"- **Resolution:** {meta['width']}x{meta['height']} ({meta.get('codec') or 'unknown codec'})"
        )
    mode = "focused" if focused else "full"
    print(
        f"- **Frames:** {len(frames)} @ {fps:.3f} fps, {mode} mode (budget {target}, max {max_frames})"
    )
    ...
    print()
    print(
        "**Inspect each frame path below with the local image-viewing tool.** "
    ...
    if transcript_text:
        ...
        if focused:
            print(
                f"_Source: {label}. Filtered to {format_time(effective_start)} → {format_time(effective_end)}:_"
            )
        else:
        ...
        print(
            f"_No transcript lines fell inside {format_time(effective_start)} → {format_time(effective_end)}._"
        )
    else:
        setup_py = SCRIPT_DIR / "setup.py"
        print(
