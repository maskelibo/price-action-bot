---
agent: analyst
type: know_how
created: 2026-05-08
---

# Analyst Know-How

---

## Playbook: Günlük KPI Brief

Bkz. `agents/analyst.md` SOP-1.
1. Postgres `trades` tablosundan dünkü trade'ler.
2. Açık pozisyon snapshot.
3. KPI tablosu (gün, hafta, ay, çeyrek, yıl).
4. Equity + drawdown curve.
5. Per-strategy breakdown.
6. Top winners/losers.
7. Bias sorgulamaları (concentration, recency).
8. CEO brief 4 cümle özet.

---

## Playbook: Trade Post-Mortem (kayıplı)

Sınıflandırma kategorileri:
- `wrong_pattern`
- `wrong_timing`
- `wrong_size`
- `regime_change`
- `data_glitch`
- `unlucky`

3+ tekrarda CEO brief'e kırmızı bayrak.

---

## Playbook: Regime Split

- Bull: BTC EMA200 üstü + 20g vol > median.
- Bear: BTC EMA200 altı.
- Range: vol < median.

Her büyük rapor 3 rejim için ayrı tablolar.

---

## Playbook: Anomaly Detection

- Returns 3σ dışı → bağlam analizi.
- Pattern hit-rate ±2σ dışı → Researcher uyarı.
- Slippage 2x ortalama → Execution + Ops ticket.

---

> Yeni şablon çıktığında ekle.

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:8

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:10

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:12

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:26

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:28

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:30

---

### 2026-05-25 — weekly-tag-snapshot-20260525 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:32

---

### 2026-05-26 — weekly-tag-snapshot-20260526 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:36

---

### 2026-05-26 — weekly-tag-snapshot-20260526 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:38

---

### 2026-05-26 — weekly-tag-snapshot-20260526 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:40

---

### 2026-05-26 — weekly-tag-snapshot-20260526 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:42

---

### 2026-05-26 — weekly-tag-snapshot-20260526 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:44

---

### 2026-05-31 — weekly-tag-snapshot-20260531 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:54

---

### 2026-06-07 — weekly-tag-snapshot-20260607 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:14

---

### 2026-06-14 — weekly-tag-snapshot-20260614 (med)
- tags: consolidation

Haftalık episodic tag dağılımı: llm_call:12

---
