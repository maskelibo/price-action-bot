"""Engulfing continuation: trade engulfing patterns in session-aligned trend direction."""
...
from ..indicators.price_action import bullish_engulfing, bearish_engulfing
from .base import Strategy, StrategyManifest


class EngulfingSessionStrategy(Strategy):
    name = "engulfing_session"
    ...
    def __init__(self, manifest=None):
        super().__init__(manifest or StrategyManifest(
            ...
            params={"trend_filter": True, "ema_for_trend": 20},
        ))
    ...
    def generate_signals(self, df: pd.DataFrame, pair: str) -> list[Signal]:
        ...
        bull = bullish_engulfing(df) & sess_ok
        bear = bearish_engulfing(df) & sess_ok
        ...
        for ts in df.index[bull.fillna(False)]:
            ...
            sl = float(min(row["low"], row["open"]) - 0.3 * a)
            if entry - sl <= 0:
                continue
            tp1 = entry + (entry - sl) * 1.5
            tp2 = entry + (entry - sl) * 3.0
            out.append(self._emit(
                pair, ts, "long", entry, sl, [tp1, tp2], 0.63, row["session"],
                "engulfing_bull", price_to_pips(pair, entry - sl),
        ...
        for ts in df.index[bear.fillna(False)]:
            ...
            if not np.isfinite(a) or a <= 0:
                continue
            entry = float(row["close"])
            sl = float(max(row["high"], row["open"]) + 0.3 * a)
            if sl - entry <= 0:
                continue
            tp1 = entry - (sl - entry) * 1.5
            tp2 = entry - (sl - entry) * 3.0
            out.append(self._emit(
                pair, ts, "short", entry, sl, [tp1, tp2], 0.63, row["session"],
                "engulfing_bear", price_to_pips(pair, sl - entry),
                meta={"pattern_hit": True, "structure_hit": False, "smc_hit": False,
                      "volume_score": float(row.get("participation", 0.5)) if pd.notna(row.get("participation")) else 0.5,
                      "trend_aligned": True},
            ))
