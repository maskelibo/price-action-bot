...
Girişte stop mesafesi en az `%2,5`, confluence en az `0,25` olmalıdır.
...
yoktur. `>30m` gap özellik warmup'ını kırar; açık pozisyon ilk işlem gören
open'da kötümser biçimde kapatılır.
...
`%1`'dir. Stop mesafesi normalizasyonu `0,01 / stop_pct` formülünü `0,20–1,50`
...
pozisyondur. Primary-13 nedeniyle fiilî toplam tavan 13'tür. Portföy notional
tavanı `3×`, margin proxy kaldıracı `3×`'tir. Causal 90 günlük günlük-return
korelasyon kapısı `0,70` üzerinde riski `×0,50` kısar, `0,90`'da girişi
engeller. Gerekli initial margin `notional / leverage` olarak hesaplanır ve
free margin'in ancak `%90`'ına kadar kabul edilir; RiskOfficer sonrasında
wrapper aynı `%90` kapısını tekrar uygular. Ayrıca stop mesafesi yaklaşık
likidasyon mesafesinin yarısını aşamaz:
`stop_pct <= (1 / leverage_used) × 0,50`.

Breaker'lar stateful ve UTC tabanlıdır:

- günlük `%4`: journal'daki partial ve final exit net PnL; ayrı funding kaydı bu
  realized feed'e side-atfedilmez, bir gün yeni giriş duruşu;
...
- aylık long `%12` ve short `%4`: ay başı wallet-balance paydasında journal
  partial/final side-PnL; ayrı funding side'a yazılmaz, yalnız ilgili yönü üç
  gün durdurur;
- beş ardışık net-kayıp episode: bir gün duruş.

Funding yine wallet'ı ve dolayısıyla weekly/monthly combined breaker'ı etkiler.
...
Raw kanıt en az şunları içerir: tüm sinyal ve ret nedenleri, karar/scheduled
