"""v0.9 - Volume strategy bireysel test 11 sembol multi-target engine.

Mevcut volume stratejiler:
  - cvd_spike_fade
...
def replay(trades, risk_pct=0.015, max_concurrent=8, cooldown_days=3):
...
def main():
    print("=" * 90)
    print("v0.9 STRATEGY SCREEN — 11 sembol + multi-target engine")
    print("=" * 90)
    print(f"\n{'Strateji':<32} {'sinyal':>7} {'final$':>10} {'yIllIk':>8} {'maxDD':>7} {'WR':>5} {'avgR':>6}")
    print("-" * 80)

    sresults = {}
    for module_name, class_name in VOLUME_STRATS:
        if module_name in sresults: continue
        trs = _gather(module_name, class_name)
        if not trs or len(trs) < 10:
            print(f"{module_name:<32} az sinyal ({len(trs)})")
            continue
        r = replay(trs, risk_pct=0.015)
    ...
    for name, ts, risk in scenarios:
        ...
        print(f"{name:<28} {r['trades']:>7} {r['final']:>10,.0f} {ann:>+7.2f}% {r['max_dd']*100:>+5.0f}% {ra:>5.2f}")
