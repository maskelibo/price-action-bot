...
"""
# ruff: noqa: E402, SIM118, RUF005  (pre-existing stil; UI script — 2026-07-10 batch-D'de yüzeye çıktı)
...
from dotenv import load_dotenv  # (sys.path-setup sonrası — script deseni)
...
def load_paper_signals():
    return _query(PAPER_JOURNAL, "SELECT * FROM paper_signals ORDER BY ts DESC")


def load_paper_events():
    return _query(PAPER_JOURNAL, "SELECT * FROM paper_position_events ORDER BY ts DESC")


def load_paper_equity():
    return _query(PAPER_JOURNAL, "SELECT * FROM paper_equity_snapshots ORDER BY ts")


def load_testnet_signals():
    return _query(TESTNET_JOURNAL, "SELECT * FROM testnet_signals ORDER BY ts DESC")


def load_testnet_oco():
    return _query(TESTNET_JOURNAL, "SELECT * FROM testnet_oco_orders ORDER BY ts DESC")


def load_testnet_equity():
    return _query(TESTNET_JOURNAL, "SELECT * FROM testnet_equity_snapshots ORDER BY ts")


def load_futures_signals():
    return _query(FUTURES_JOURNAL, "SELECT * FROM futures_signals ORDER BY ts DESC")


def load_futures_protection():
    return _query(FUTURES_JOURNAL, "SELECT * FROM futures_protection_orders ORDER BY ts DESC")


def load_futures_equity():
    return _query(FUTURES_JOURNAL, "SELECT * FROM futures_equity_snapshots ORDER BY ts")
...
def status_pill(label: str, status: str) -> str:
    cls = {"live": "pill-live", "warn": "pill-warn", "stop": "pill-stop", "info": "pill-info"}.get(
        status, "pill-info"
    )
    text = {"live": "LIVE", "warn": "STALE", "stop": "OFF", "info": "INFO"}.get(
        status, status.upper()
    )
...
with st.sidebar:
    ...
    st.markdown("**Daemon**")
    st.markdown(
        status_pill("Spot", spot_st) + "<br>" + status_pill("Futures", fut_st),
        unsafe_allow_html=True,
    )
    ...
    if "_error" not in fut_state:
        st.metric(
            "🚀 Futures",
            f"${fut_state['wallet_balance']:,.0f}",
            delta=f"{fut_state['unrealized_pnl']:+.2f}",
        )
        st.metric(
            "📍 Pozisyon",
            f"{fut_state['n_positions']}",
...
if page == "📊 Genel Bakış":
    ...
    with c1:
        if "_error" not in fut_state:
            ...
            st.metric("🚀 Futures Wallet", f"${wallet:,.2f}", delta=f"${wallet-5000:+,.0f} vs $5k")
        else:
    ...
    with c2:
        if "_error" not in fut_state:
            st.metric(
                "📊 Unrealized PnL",
                f"${fut_state['unrealized_pnl']:+,.2f}",
                delta=f"{fut_state['n_positions']} açık poz",
            )
    with c3:
        st.metric("🔌 Spot USDT", f"${spot_usdt:,.2f}", delta=f"${spot_usdt-10000:+,.0f} vs $10k")
    with c4:
        if "_error" not in fut_state:
            st.metric(
                "🎯 Algo Orders",
                f"{fut_state['n_algo_orders']}",
                delta="TP+SL koruma",
                delta_color="off",
            )
    with c5:
        ...
        st.metric(
            "📜 Futures Trade", f"{n_filled}", delta=f"{len(sigs)} toplam sinyal", delta_color="off"
        )
    ...
    with col1:
        ...
        if not eq_f.empty:
            ...
            fig.add_hline(
                y=5000, line_dash="dash", line_color="gray", annotation_text="$5k başlangıç"
            )
            fig.update_layout(
                height=260,
    ...
    with col2:
        ...
        if not eq_s.empty:
            ...
            fig.add_hline(
                y=10000, line_dash="dash", line_color="gray", annotation_text="$10k başlangıç"
            )
            fig.update_layout(
                height=260,
                margin=dict(l=10, r=10, t=20, b=20),
...
# ============================================================
    ...
    if not eq_f.empty:
        ...
        fig.add_hline(
            y=5000, line_dash="dash", line_color="#475569", annotation_text="$5k başlangıç"
        )
        fig.update_layout(
            height=320,
            margin=dict(l=10, r=10, t=20, b=20),
    ...
    if not sigs.empty:
        ...
        st.dataframe(
            sigs_show[cols_avail].head(50), use_container_width=True, hide_index=True, height=400
        )
    else:
...
# ============================================================
    ...
    if bal:
        df_bal = (
            pd.DataFrame([{"Currency": k, "Amount": v} for k, v in bal.items() if v > 0.0001])
    ...
    if not oco.empty:
        ...
        if not active.empty:
            ...
            st.dataframe(
                active[["ts", "symbol", "qty", "tp_price", "sl_trigger", "sl_limit"]].rename(
                    columns={
                        "ts": "Açılış",
                        "symbol": "Sembol",
                        "qty": "Qty",
...
# ============================================================
    ...
    with tab2:
        ...
        if tn.empty:
            ...
            c1, c2 = st.columns(2)
            f_st = c1.selectbox(
                "Status", ["Hepsi"] + sorted(tn["status"].unique().tolist()), key="tnst"
            )
            f_sym = c2.selectbox(
                "Sembol", ["Hepsi"] + sorted(tn["symbol"].unique().tolist()), key="tnsy"
            )
            f = tn.copy()
            if f_st != "Hepsi":
                f = f[f["status"] == f_st]
...
# ============================================================
    ...
    if sigs.empty:
        ...
        side_counts = (
            sigs[sigs["status"] == "filled"].groupby("side").size().reset_index(name="count")
        )
        if not side_counts.empty:
            ...
            fig.update_layout(
                height=350, plot_bgcolor="#0f172a", paper_bgcolor="#0f172a", font_color="#e2e8f0"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        if not by_sym.empty:
            fig = px.bar(
                by_sym.sort_values("trade", ascending=True),
                x="trade",
                y="symbol",
                orientation="h",
                color="trade",
...
# ============================================================
    ...
    fig2.add_trace(
        go.Bar(
            ...
            y=history["DD"].abs(),
            marker_color="#ef4444",
            text=history["DD"].apply(lambda x: f"%{x:.1f}"),
            textposition="outside",
        )
    )
    fig2.update_layout(
