"""SEC27: Phoenix v2.0.4 4h Primary Timeframe Pivot.

Hipotez: SEC6 (2026-05-13) v0.9.7 BALANCED'da 4h pivot RED (yıllık +14.4% vs +33.6%).
v2.0.4 PHOENIX (pyramid + side-cond DD + mc=12 + FVG) farklı dinamiklere sahip — sonuç
direkt port edilemez. Bu sprint: PHOENIX config + 4h ile retest.

Champion reference: Phoenix v2.0.4 1d = yıllık +%200.3 / DD -%32 / r-adj 6.26 / WR %69.7
(13 pencere, 13/13 pozitif).

Adımlar:
1. PHOENIX_STRATEGIES (10) × SYMBOLS_10 (4h pool, MATIC yok) trade collect (peak_R enriched)
2. configs/risk_phoenix_v204.yaml ile production_replay
3. Rolling walk-forward (2y train + 3mo OOS + 1mo step — 4h pool 3y)
4. Phoenix 1d vs Phoenix 4h karşılaştırma

Output: reports/lab/sec27_phoenix_4h_pivot_2026-05-16.md
"""
...
REPORT_OUT = ROOT / "reports" / "lab" / "sec27_phoenix_4h_pivot_2026-05-16.md"
...
def _gather_4h_peakR(module_name: str, class_name: str) -> list:
    """Phoenix 4h gather — peak_R enriched (pyramid için kritik)."""
    try:
    ...
    for sym in SYMBOLS_10:
        try:
            df = _load_symbol_ohlcv(sym, tf="4h")
...
def main() -> None:
    ...
    w("# SEC27: Phoenix v2.0.4 — 4h Primary Timeframe Pivot")
    w("")
    ...
    w("")
    w("## Trade Toplama (10 Phoenix strateji × 10 sym × 4h)")
    w("")
    print("Trade topluyor (Phoenix × 4h)...")
    ...
    w("")
    w(f"**Toplam 4h trade pool:** {len(all_trades)}")
    w(f"**Phoenix 1d baseline:** ~4787 trade (SEC6 v0.9.7), v2.0.4 daha fazla (FVG)")
    if len(all_trades) > 0:
    ...
    if len(all_trades) < 100:
        w("")
        w("**HATA:** 4h pool yetersiz (< 100 trade).")
    ...
    w("")
    w(f"**Pencere sayısı:** {len(windows)} (2y rolling, 30 gün step — 4h pool 3y)")
    w(f"**4h pool aralığı:** {start} -> {end}")
    w("")
    ...
    # Senaryolar:
    # - PHOENIX 4h default (yaml as-is: mc=12, pyramid, side-cond)
    # - PHOENIX 4h max_conc=20 (4h pool büyük, slot artırılınca)
    # - PHOENIX 4h cooldown=0 (4h cooldown 1d=3 ayni iken 12 bar gibi davranır → çok dar)
    scenarios = [
        ("PHOENIX 4h default (mc=12, pyramid, side-cond)",
         all_trades,
         phoenix_cfg),
        ("PHOENIX 4h max_conc=20",
         all_trades,
         phoenix_cfg.with_overrides(max_concurrent=20)),
        ("PHOENIX 4h mc=20 + cooldown=0",
         all_trades,
