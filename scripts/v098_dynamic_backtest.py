...
"""
# ruff: noqa: E402  (ölü script — env-setup import deseni, 2026-07-10)
...
from scripts.v09_optimize_top10 import TOP_10, _gather
...
def main():
    ...
    presets = [
        ...
        (
            "DYNAMIC v0.9.8 ⭐",
            "configs/risk_dynamic.yaml",
        ),  # ÖLÜ REFERANS: configs/risk_dynamic.yaml mevcut değil (arşivde) — koşulursa crash. Doğrulandı 2026-07-10
    ]

    # 3y rolling 13 pencere
    print("\n# 3Y ROLLING 13 PENCERE")
    print(
        f"  {'preset':<32} {'yillik':>8} {'med':>7} {'min':>7} {'max':>7} {'DD':>6} {'minDD':>7} {'r-adj':>6}"
    )
    print("-" * 100)
    ...
    for name, yml in presets:
        ...
        print(
            f"  {name:<32} {ma:>+7.1f}% {median(anns):>+6.1f}% {min(anns):>+6.1f}% {max(anns):>+6.1f}% {md:>+5.0f}% {min(dds):>+6.0f}% {ra:>5.3f}"
        )
    ...
    for name, yml in presets:
        ...
        print(
            f"  {name:<32} {r.final_equity:>10,.0f} {ann:>+7.2f}% {r.max_drawdown*100:>+6.1f}% {r.win_rate*100:>4.0f}% {r.trades:>4}"
        )
