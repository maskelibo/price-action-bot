"""SEC25 - Brooks Double Bottom Bull Flag / Double Top Bear Flag standalone backtest.

Pre-registered hipotez:
  memory/researcher/hypotheses/2026-05-14-brooks-double-bottom-bull-flag.md
...
Orthogonality kontrolu:
  Trigger-date jaccard with engulfing_continuation, donchian_breakout
  (pre-reg counter-hypotheses CH-2, CH-3).
"""
...
STRATEGY_MODULE = "brooks_db_bull_flag"
STRATEGY_CLASS = "BrooksDBBullFlagStrategy"
...
def _gather_strategy(module_name, class_name, universe):
    """Gather trades for any strategy module (used for our strategy + orthogonality)."""
...
def _orthogonality_check(my_trades, universe):
    """Trigger-date overlap with engulfing_continuation and donchian_breakout.

    Pre-reg counter-hypotheses CH-2 (engulfing < 0.20) and CH-3 (donchian < 0.30).
    """
    if not my_trades:
        return {}
    out = {}
    ...
    for other_name, other_cls in others.items():
        try:
            mod = __import__(f"price_action.strategies.{other_name}",
                            fromlist=[other_cls, "_default_manifest"])
            cls_obj = getattr(mod, other_cls, None)
            if cls_obj is None:
                # Try to discover
                for attr in dir(mod):
                    if attr.endswith("Strategy") and not attr.startswith("_"):
                        ...
                        break
            manifest_fn = getattr(mod, "_default_manifest", None)
            if manifest_fn is None or cls_obj is None:
                out[other_name] = None
                continue
            strategy = cls_obj(manifest_fn())
            other_dates = set()
            for sym in universe:
                df = _load_symbol_ohlcv(sym, tf="1d")
                if df is None or df.empty:
...
def main():
    print("=" * 100)
    print("SEC25 - Brooks Double Bottom Bull Flag / Double Top Bear Flag standalone")
    ...
    if sc != "trend_continuation":
        print(f"  FAIL - STRATEGY_CLASS = {sc!r} (expected 'trend_continuation')")
        return
    print(f"  PASS - STRATEGY_CLASS = 'trend_continuation'")
    print()

    print("[GATHERING TRADES...]")
    trades = _gather_strategy(STRATEGY_MODULE, STRATEGY_CLASS, SYMBOLS_11)
    ...
    # Orthogonality (CH-2, CH-3)
    print("[ORTHOGONALITY - trigger-date overlap with engulfing_continuation / donchian_breakout]")
    ...
    if path.exists():
        ...
        # Note: shift(-n) IS USED for fractal forward-window; this is allowed
        # because we only QUERY is_swing_low[k] when k+fractal_n <= t (confirmed).
        # The is_swing_low column itself uses shift(-n) but we never consume it
        # at the same bar as the swing — only at t after fractal_n bars later.
        # We DO still flag it for human review.
        bad_patterns = [
            r"\.shift\(-\d+\)",
        ...
        if flags:
            ...
            print(f"  NOTE: shift(-n) for fractal forward-confirm is valid IF")
            print(f"        only queried at t after fractal_n confirmation (see signal loop).")
        else:
            print(f"  {STRATEGY_MODULE}: clean")
    print()
    ...
    summary_path = out_dir / "sec25_brooks_db_bull_flag_standalone.json"
    ...
    pkl_path = out_dir / "sec25_brooks_db_bull_flag_trades.pkl"
